...
export default {
  ...
  computed: {
    ...
    getLineColor() {
      return this.lineColor || '#5677fc';
    },
    getCkMarkColor() {
      return this.checkMarkColor || '#5677fc';
    },
  },
  watch: {
    itemList(val) {
  ...
  methods: {
    setDefaultData(val) {
      ...
      if (defaultItemList.length > 0) {
        if (
          (typeof defaultItemList[0] === 'string' ||
            typeof defaultItemList[0] === 'number') &&
          !this.request
        ) {
          let subi = -1;
          let selectedArr = [];
      ...
      if (!data || data.length === 0) {
        ...
        this.$nextTick(() => {
          setTimeout(() => {
            ...
            this.scrollViewId = `id_${
              this.currentTab > 1 ? this.currentTab - 1 : 0
            }`;
          }, 50);
        });
      }
