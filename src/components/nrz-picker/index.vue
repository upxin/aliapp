<template>
  <view
    ...
    :class="[visible ? 'tui-picker__mask-show' : '']"
    :style="{ zIndex }"
    @tap="maskClick"
  >
  </view>
  <view
    ...
    :style="{ backgroundColor: backgroundColor, zIndex: zIndex + 1 }"
    :class="[
      visible ? 'tui-picker__show' : '',
      radius ? 'tui-picker__radius' : '',
    ]"
  >
    <view
      class="tui-picker__header"
    ...
    <view @touchstart.stop="pickstart">
      <div
        v-if="showCurrent"
        class="flex justify-between m-auto bg-fff px-30px py-16px"
      >
        <span
          class="box-border text-center text-14px nr-border rounded-sm p-8px"
          @click="handleCurrent"
        >
          {{ currentCityName ? '当前位置：' + currentCityName : '定位失败' }}
        </span>
        <span class="flex items-center" @click="handlePosi">
      ...
      >
        <picker-view-column>
          <view
            v-for="(item, index) in layer1__data"
            :key="index"
            :style="{ color: color }"
            class="tui-picker__item"
            >{{ item }}</view
          >
        </picker-view-column>
        <picker-view-column v-if="layer == 2 || layer == 3">
          <view
            v-for="(item, index) in layer2__data"
            :key="index"
            :style="{ color: color }"
            class="tui-picker__item"
            >{{ item }}</view
        ...
        <picker-view-column v-if="layer == 3">
          <view
            v-for="(item, index) in layer3__data"
            :key="index"
            :style="{ color: color }"
            class="tui-picker__item"
            >{{ item }}</view
...
export default {
  ...
  emits: ['pickstart', 'pickend', 'hide', 'change', 'update:show'],
  data() {
    return {
      visible: false,
      vals: [0],
  ...
  computed: {
    getConfirmColor() {
      return this.confirmColor || '#5677fc';
    },
    currentCityName() {
      return store().cityInfo.currentCityName;
    },
  },
  watch: {
    show(val) {
  ...
  methods: {
    handlePosi() {
      ...
      useLocation()
        .then(({ longitude, latitude, ...rest }) => {
          console.log('picker then', longitude, latitude);
          ...
          getCityInfo({
            ...
            .catch(() => {
              console.log('getCityInfo catch');
              Taro.hideLoading();
            });
        })
        .catch(() => {
          console.log('picker catch');
    ...
    handleCurrent() {
      this.$emit('change', {
        ...
        value: ['', store().cityInfo.adcode],
        result: '',
        center: [
          '',
          `${store().cityInfo.latitude},${store().cityInfo.longitude}`,
        ],
      });
      this.$emit('update:show', false);
    },
    ...
    getValue(key = 'text', layer = 1) {
      ...
      if (layer === 1) {
        ...
        if (
          data[vals[0]].children &&
          data[vals[0]].children[vals[1]].children
        ) {
    ...
    pickerChange() {
      ...
      if (this.pickerData.length > 0) {
        if (this.layer == 1) {
          ...
          result = text;
          center = this.getValue('center');
        } else if (this.layer == 2) {
          text = [this.getValue(), this.getValue('text', 2)];
          value = [this.getValue('value'), this.getValue('value', 2)];
          result = text.join('');
          center = [this.getValue('center'), this.getValue('center', 2)];
        } else {
          text = [
            this.getValue(),
            this.getValue('text', 2),
            this.getValue('text', 3),
          ];
          value = [
            this.getValue('value'),
            this.getValue('value', 2),
            this.getValue('value', 3),
          ];
          result = text.join('');
          center = [
            this.getValue('center'),
            this.getValue('center', 2),
            this.getValue('center', 3),
          ];
        }
      }
    ...
    setDefaultOptions() {
      ...
      if (this.layer1__data.length > 0 && textArr.length > 0) {
        textArr.forEach((item, idx) => {
          ...
          if (idx === 0) {
            this.layer2__data = this.toArr(
              this.checkChildrenData(this.pickerData, 1, index)
            );
          } else if (idx === 1) {
            this.layer3__data = this.toArr(
              this.checkChildrenData(this.pickerData, 2, vals[0], index)
            );
          }
          if (index === -1) {
            vals.push(0);
        ...
        if (this.layer == 1) {
          ...
          vals = [0, 0];
          this.layer2__data = this.toArr(
            this.checkChildrenData(this.pickerData, 1, 0)
          );
        } else {
          vals = [0, 0, 0];
          this.layer2__data = this.toArr(
          ...
          this.layer3__data = this.toArr(
            this.checkChildrenData(this.pickerData, 2, 0, 0)
          );
        }
      }
      this.vals = vals;
    ...
    initData(layer, index, idx) {
      ...
      if (this.layer == 1) {
        ...
        if (layer === 0 || layer === -1)
          this.layer2__data = this.toArr(
            this.checkChildrenData(data, 1, index)
          );

        this.layer3__data = this.toArr(
          this.checkChildrenData(data, 2, index, idx)
        );
      }
    },
