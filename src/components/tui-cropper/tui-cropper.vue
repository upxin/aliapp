...
export default {
  ...
  computed: {
    imgTransform: function () {
      return `translate3d(${this.imgLeft - this.imgWidth / 2}px,${
        this.imgTop - this.imgHeight / 2
      }px,0) scale(${this.scale}) rotate(${this.angle}deg)`;
    },
  },
  watch: {
    imageUrl(val, oldVal) {
  ...
  mounted() {
    ...
    this.ctx = Taro.createCanvasContext('tui-image-cropper', this);
    this.setCutCenter();
  ...
  methods: {
        ...
        this.ctx.draw(false, () => {
          ...
          if (this.isBase64) {
            this.ctx.toDataURL(params).then((dataURL) => {
              data.base64 = dataURL;
              this.loadding && Taro.hideLoading();
              ...
              this.ctx.clearRect(
                0,
                0,
                this.canvasWidth * this.scaleRatio,
                this.canvasHeight * this.scaleRatio
              );
              this.ctx.draw();
     ...
     */
    setCutCenter() {
      let sys = this.sysInfo || Taro.getSystemInfoSync();
