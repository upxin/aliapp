<template>
  <view v-if="isFixed" @touchmove.stop.prevent="stop">
    <view class="nrz-bottom-popup" :class="{ 'nrz-popup-show': isShow }">
      <view
        class="nrz-calendar-header"
        :class="{ 'nrz-calendar-radius': radius }"
      >
        <view>{{ title }}</view>
        <view
          class="nrz-iconfont nrz-font-close"
      ...
      <view
        ...
        :class="{ 'nrz-flex-start': isFixed && fixedHeight }"
        :style="{
        ...
        >
          ...
          >
            ...
            <view
              ...
              class="nrz-custom-desc"
              :class="{ 'nrz-lunar-unshow': !lunar && isFixed }"
            >
      ...
      <view class="nrz-calendar-op">
        <view class="nrz-calendar-result">
          <text>{{ type == 1 ? activeDate : startDate }}</text>
          <text v-if="endDate" style="padding: 0 4px">至</text>
          <text v-if="endDate">{{ endDate }}</text>
        </view>
        <view class="nrz-calendar-btn_box flex justify-between px-40px">
          <button
            v-if="extraBtn"
            style="padding: 10px 0"
            class="text-14px text-fff flex-1 border-0 mx-10px bg-primary rounded-none py-10px"
            @click="getAll"
          >
            所有历史
          </button>
          <button
            style="padding: 10px 0"
            class="text-14px flex-1 text-fff border-0 mx-10px bg-primary rounded-none py-10px"
            :disabled="disabled"
            @click="btnFix(false)"
          >
            确定
    ...
    <view
      class="nrz-popup-mask"
      :class="[isShow ? 'nrz-mask-show' : '']"
      @tap="hide"
    ></view>
  </view>
  <view v-else>
...
<script>
//easycom组件模式 无需手动引入
// import tuiButton from "../nrz-button/nrz-button.vue"
...
export default {
  ...
  props: {
    ...
    maxYear: {
      type: Number,
      default: dayjs().add(10, 'years').year(),
    },
    //可切换最小年份
    minYear: {
      type: Number,
