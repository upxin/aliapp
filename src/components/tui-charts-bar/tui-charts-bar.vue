<template>
  <view class="tui-charts__bar-wrap" :style="{ width: width + 'rpx' }">
    <view class="tui-bar__legend" v-if="legend.show">
      <view
        class="tui-bar__legend-item"
        v-for="(item, index) in dataset"
        :key="index"
      >
        <view
          class="tui-legend__circle"
    ...
    <view
      class="tui-charts__bar-box"
      :style="{
        borderLeftColor: yAxisLine.color,
        borderBottomColor: xAxisLineColor,
      }"
    ...
    >
      ...
      <view class="tui-bar__yAxis-linebox">
        <view
          ...
          :key="idx"
          :style="{
            borderLeftStyle: splitLine.type,
            borderLeftColor: splitLine.color,
          }"
        >
          <text
            class="tui-xAxis__val"
            :style="{
    ...
    <view
      class="tui-column__tooltip"
      v-if="tooltip"
      :class="{ 'tui-column__tooltip-show': tooltipShow }"
    >
      <view class="tui-tooltip__title">{{ yAxis[activeIndex] || '' }}</view>
      <view
        class="tui-column__tooltip-item"
...
export default {
  ...
  methods: {
    getBarWidth(value, dataset) {
      ...
      return (
        ((value - (this.isStack ? min / dataset.length : min)) /
    ...
    getXAxisVal(index) {
      ...
      if (
        this.yAxisVal.formatter &&
        typeof this.yAxisVal.formatter === 'function'
      ) {
        showVal = this.yAxisVal.formatter(val);
      } else if (
        this.yAxisValFormatter &&
        typeof this.yAxisValFormatter === 'function'
      ) {
    ...
    getLeft(index) {
      ...
      return (
        (((this.isStack ? total : Math.max(...arr)) - this.min) /
          (this.maxValue - this.min)) *
          this.width +
        'rpx'
      );
    ...
    init() {
      console.log(this.max, this.splitNumber, 888888);
